...
(function() {
  var ExtendedSlider, slider;

  ExtendedSlider = (function() {
    function ExtendedSlider(config) {
      this.config = config;
    ...
    ExtendedSlider.prototype.addImgs = function() {
      var i, index, j, len, ref, results;
      $('.fotoPrall').html("").hide();
      ref = this.config[this.curindex].sub_img;
      results = [];
      for (index = j = 0, len = ref.length; j < len; index = ++j) {
        i = ref[index];
        results.push($('.fotoPrall').append("<a class='galsl' href='" + this.config[this.curindex].full_img[index] + "'> <div class='photo_pr' style='background:url(" + i + ") no-repeat;background-size:contain'></div>").fadeIn(800));
      }
      return results;
    };

    ExtendedSlider.prototype._bindEvents = function() {
      $('.arrow_small_left').click((function(_this) {
        return function() {
          _this.slider.goToPrevSlide();
          if (_this.curindex === 0) {
            _this.curindex = _this.config.length - 1;
          } else {
            _this.curindex--;
          }
          _this.addImgs();
      ...
      $('.arrow_small_right').click((function(_this) {
        return function() {
          ...
          if (_this.curindex + 1 === _this.config.length) {
            _this.curindex = 0;
          } else {
            _this.curindex++;
          }
  ...
  slider = new ExtendedSlider([
    {
      ...
      main: "Проект 3",
      text: "text3",
      sub_img: ["images/photo4.png", "images/photo4.png", "images/photo4.png"],
      full_img: ["proj/fireplace/img11.jpg", "proj/fireplace/img11.jpg", "proj/fireplace/img11.jpg"]
    }
  ]);
...
(function() {
  $(document).ready(function() {
    ...
    true;
    return $(document).mousewheel(function(e) {
      if (e.pageY > window.innerHeight && flag) {
        flag = false;
        $('.horMenu').animate({
          'opacity': 1
        }, 100);
